...
using Game; // For StudySet, Flashcard
...
namespace FashcardGame_FinalProject_WPF_C_.viewModels
{
    public class QuizViewModel : BaseViewModel
    {
        ...
        private List<Game.Flashcard> _allFlashcards;
        private List<Game.Flashcard> _questionsToAsk;
        private List<Game.Flashcard> _incorrectlyAnswered;
        private Game.Flashcard? _currentFlashcard;
        ...
        private HashSet<Game.Flashcard> _alreadyAttemptedFirstTry = new HashSet<Game.Flashcard>();
        private HashSet<Game.Flashcard> _alreadyAttempted = new HashSet<Game.Flashcard>();

        // Starts or restarts the quiz
        private void StartQuiz_Final()
        {
            _questionsToAsk = _allFlashcards.OrderBy(x => _random.Next()).ToList();
            _incorrectlyAnswered = new List<Game.Flashcard>();
        ...
        public QuizViewModel(StudySet studySet)
        {
            ...
            _allFlashcards = new List<Game.Flashcard>(_originalStudySet.cards ?? new List<Game.Flashcard>()); // Defensive copy
