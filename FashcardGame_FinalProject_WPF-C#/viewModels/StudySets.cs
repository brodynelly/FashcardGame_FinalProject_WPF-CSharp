...
using Game; // For Course and StudySet models
...
namespace FashcardGame_FinalProject_WPF_C_.viewModels
{
    ...
    {
        private Game.Course? _currentCourse;
        public Game.Course? CurrentCourse
        {
            get => _currentCourse;
        ...
        // Constructor that takes the selected Game.Course object
        public StudySets(Game.Course course)
        {
        ...
        {
            if (CurrentCourse?.flashSet != null)
            {
                StudySetList = new ObservableCollection<StudySet>(CurrentCourse.flashSet);
            }
            else
            {
        ...
        private void StartCourseByName()
        {
            if (!string.IsNullOrEmpty(SelectedCourseNameFromList))
            {
                ...
                // If you want to load a Game.Course object based on this name, you'd need a lookup mechanism.
            }
        }
    }
}